...
class DispatchTableManager final {
   public:
    ...
    /**
     * Looks up the instance dispatch table for a dispatchable object.
     * Returns a pointer to the stored table on success, or nullptr if not registered.
     *
     * Note: VkPhysicalDevice dispatch keys do not match VkInstance dispatch keys.
     * Resolve the parent VkInstance first (e.g. via DeviceInstanceTracker) before querying.
     */
    [[nodiscard]] VkuInstanceDispatchTable* GetInstanceDispatchTable(const void* object) const;

    /**
     * Destroys the instance dispatch table for the given instance handle.
     * Resolves the dispatch key from the handle. Do not pass an invalidated handle
     * after downstream vkDestroyInstance has executed; capture Key beforehand and invoke
     * DestroyInstanceTable(Key) instead.
     */
    void DestroyInstanceTable(VkInstance instance) {
        assert(instance != VK_NULL_HANDLE);
        DestroyInstanceTable(GetDispatchKey(instance));
    }

    /**
     * Destroys the instance dispatch table for the given dispatch key.
     * Safe to invoke after downstream vkDestroyInstance has executed.
     */
    ...
    /**
     * Destroys the device dispatch table and loader callback for the given device handle.
     * Resolves the dispatch key from the handle. Do not pass an invalidated handle
     * after downstream vkDestroyDevice has executed; capture Key beforehand and invoke
     * DestroyDeviceTable(Key) instead.
     */
    void DestroyDeviceTable(VkDevice device) {
        assert(device != VK_NULL_HANDLE);
        DestroyDeviceTable(GetDispatchKey(device));
    }

    /**
     * Destroys the device dispatch table and loader callback for the given dispatch key.
     * Safe to invoke after downstream vkDestroyDevice has executed.
     */
