...
```
                          ...
                          +-----------------------------------+
                               ...
                               v            v            v
                +---------------+  +----------------+  +----------------------+
                | LayerManifest |  | DispatchTable- |  | DeviceInstance-      |
                | Declarative   |  | Manager        |  | Tracker              |
                | Metadata      |  | Per-handle     |  | VkPhysicalDevice ->  |
                | & Extensions  |  | Dispatch Tables|  | VkInstance Mapping   |
                +---------------+  +----------------+  +----------------------+
                                            |
                                            v
                          +-----------------------------------+
...
* **`DispatchTableManager`** ([`dispatch_table_manager.h`](dispatch_table_manager.h), [`dispatch_table_manager.cpp`](dispatch_table_manager.cpp)):
  Thread-safe registry for `VkuInstanceDispatchTable` and `VkuDeviceDispatchTable` keyed by dispatchable handle. Tracks and forwards `VK_LOADER_DATA_CALLBACK` to initialize dispatchable handles created internally by layers.
* **`DeviceInstanceTracker`** ([`device_instance_tracker.h`](device_instance_tracker.h), [`device_instance_tracker.cpp`](device_instance_tracker.cpp)):
  Maintains thread-safe associations between `VkPhysicalDevice` and parent `VkInstance`. Ensures physical devices resolve their parent instance dispatch table without ambiguity or runtime type erasure.
...
TEST_F(MyCustomLayerTest, ManifestValidation) {
    const LayerManifest* manifest = MyCustomLayer::Get().GetLayerManifest();
...
```
> **Note**:
> - Do **not** link `layersvt_entrypoints` into test executables; tests link directly to `layersvt_common` and call `LayerBase` APIs directly to avoid symbol collisions with `Vulkan::Loader`.
> - Do **not** link the static `g_layer` instance into unit tests. Instantiate `g_layer` in a dedicated layer module entrypoint file so unit test fixtures can instantiate and tear down isolated instances via `layer_test::ResetLayer<MyCustomLayer>()`.
...
1. **Do Not Reimplement Common Dispatch Boilerplate**:
   Never manually parse `VkLayerInstanceCreateInfo` / `VkLayerDeviceCreateInfo` link chains or allocate raw dispatch tables. `LayerBase` automatically unwraps loader chains, initializes `DispatchTableManager`, and registers physical devices in `DeviceInstanceTracker`.
...
5. **Thread Safety**:
   `DispatchTableManager` and `DeviceInstanceTracker` are fully synchronized via mutexes. Layer-specific global states must similarly protect their own internal maps. Avoid recursive locks across downstream dispatch invocations.
