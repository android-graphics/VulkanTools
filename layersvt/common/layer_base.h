 ...
 * - Thread Safety: Internal registries (DispatchTableManager, DeviceInstanceTracker)
 *   are thread-safe. Overridden hooks called concurrently by Vulkan applications must
 *   maintain their own thread safety for layer-specific state.
...
class LayerBase {
   ...
   private:
    ...
    [[nodiscard]] DeviceInstanceTracker& GetDeviceTracker() noexcept { return device_tracker_; }
    [[nodiscard]] const DeviceInstanceTracker& GetDeviceTracker() const noexcept { return device_tracker_; }
    ...
    DeviceInstanceTracker device_tracker_;
