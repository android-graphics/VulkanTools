...
TEST(LayerBaseEnumerationTest, InstanceExtensions) {
    ...
    uint32_t count = 0;
    VkResult result = LayerBaseTestPeer::EnumerateInstanceExtensionProperties(nullptr, &count, nullptr);
    EXPECT_EQ(result, VK_SUCCESS);
    EXPECT_EQ(count, 1u);
...
TEST(LayerBaseHooksTest, ProcessInstanceExtensionsAugmenting) {
    ...
    std::vector<VkExtensionProperties> extensions(count);
    result = LayerBaseTestPeer::EnumerateInstanceExtensionProperties(nullptr, &count, extensions.data());
    EXPECT_EQ(result, VK_SUCCESS);
    EXPECT_EQ(count, 2u);
...
TEST(LayerBaseEnumerationTest, EmptyExtensionsWithBuffer) {
    ...
    VkResult result = LayerBaseTestPeer::EnumerateInstanceExtensionProperties(nullptr, &count, properties);
    EXPECT_EQ(result, VK_SUCCESS);
    EXPECT_EQ(count, 0u);
