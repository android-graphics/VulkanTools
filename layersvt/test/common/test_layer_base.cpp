...
TEST(LayerBaseTest, CreateInstanceWithMockChain) {
        ...
        if (std::strcmp(function_name, "vkEnumeratePhysicalDevices") == 0) {
            return reinterpret_cast<PFN_vkVoidFunction>(
                +[](VkInstance, uint32_t* physical_device_count, VkPhysicalDevice* physical_devices) -> VkResult {
                    ...
                    physical_devices[0] = reinterpret_cast<VkPhysicalDevice>(static_cast<uintptr_t>(0x9999));
                    *physical_device_count = 1;
                    return VK_SUCCESS;
                });
        }
    ...
    EXPECT_EQ(LayerBaseTestPeer::GetDeviceTracker(layer).GetVkInstance(
                  reinterpret_cast<VkPhysicalDevice>(static_cast<uintptr_t>(0x9999))),
              instance);
...
TEST(LayerBaseTest, PreCreateDeviceMutation) {
    ...
    MutatingLayer layer;
    LayerBaseTestPeer::GetDeviceTracker(layer).SetVkInstance(mock_physical_device, mock_instance);
...
TEST(LayerBaseTest, TeardownOrdering) {
    ...
    EXPECT_EQ(LayerBaseTestPeer::GetDeviceTracker(layer).GetVkInstance(mock_physical_device), VK_NULL_HANDLE);
}
...
TEST(LayerBaseTest, EnumeratePhysicalDevicesMapping) {
    ...
    EXPECT_EQ(LayerBaseTestPeer::GetDeviceTracker(layer).GetVkInstance(mock_physical_device), mock_instance);
}
...
TEST(LayerBaseTest, EnumeratePhysicalDeviceGroupsMapping) {
    ...
    EXPECT_EQ(LayerBaseTestPeer::GetDeviceTracker(layer).GetVkInstance(mock_physical_device1), mock_instance);
    EXPECT_EQ(LayerBaseTestPeer::GetDeviceTracker(layer).GetVkInstance(mock_physical_device2), mock_instance);
}
